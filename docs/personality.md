...
对话范本，帮助模型对齐角色语气（放在 prompt 尾部，预算紧张时会被优先截断，不影响主体）：
...
> 建议在每条中加入具体 QQ 号，Bot 才会在对话中正确区分主人与其他用户。主人称呼会从核心记忆中自动解析（`【...】` 内且非纯数字的内容），无需再写死。
...
A：直接写数字即可，例如 `QQ号【2461292801】`。多个主人的情况可以写多条。主人称呼请放在 `【...】` 内（如 `称呼为【龙玄月】与【龙星梦】`），Bot 会自动解析，无需在其他地方重复填写。

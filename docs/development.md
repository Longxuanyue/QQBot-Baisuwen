...
| `/国际象棋 [教程]` | 私聊人机国际象棋：开始/查看练习局（可用 白\|黑 与 简单\|普通\|困难 参数） | chess |
| `/国际象棋 <白\|黑> <难度>` | 自定义练习局（难度：简单/普通/困难，不计分） | chess |
| `/挑战 <对手>` | 计分挑战局（青铜/白银/黄金/钻石/王者/终极，Elo 段位分） | chess |
| `/棋局` | 查看当前棋盘（对局中直接发走法如 e4 亦可） | chess |
| `/悔棋` | 撤销一步（仅练习局） | chess |
| `/认输`、`/投降` | 认输结束对局 | chess |
| `/退出棋局` | 放弃当前对局（挑战局按认输计分） | chess |
| `/排行榜` | 段位分排行榜（玩家按分数排名） | chess |
| `/我的分数` | 个人分数、战绩与最高挑战 | chess |
